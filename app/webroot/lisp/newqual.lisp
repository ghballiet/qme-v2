...
; SAVE-CONTENT inputs a list of types and prints out the instances
; of each type in a readable format. 

(defun save-content (file) 
  (with-open-file (stream file
			  :direction :output
			  :if-exists :supersede
			  :if-does-not-exist :create)
		  (save-places stream)
		  (save-quantities stream)
		  (save-claims stream)
		  (save-facts stream)
		  (format stream "~%")
		  (format stream "~a" "done")
		  (format stream "~%")
		  (close stream))
  (say file " saved")(terpri) nil)

(defun save-places (stream)
  (mapc #'(lambda (p) (format stream "~%")(save-place p stream))
	(reverse places*)))  

(defun save-place (place stream)
  (format stream "~a" "place ")
  (format stream "~a" (place-name place))
  (cond ((not (null (place-in place)))
	 (format stream "~a" " in the ")
	 (format stream "~a"
		 (get-place-name-by-id (place-in place) places*))))
  (cond ((not (null (place-note place)))
	 (format stream "~%")(format stream "~a" "note")
	 (mapc #'(lambda (n) (format stream "~a" n)(format stream "~a" " "))
	       (place-note place)))))

(defun save-quantities (stream)
  (mapc #'(lambda (e)
	    (cond ((not (eq (quantity-name e) 'time))
		   (format stream "~%")(save-quantity e stream))))
	(reverse quantities*)))

(defun save-quantity (quantity stream)
  (format stream "~a" (quantity-type quantity))
  (format stream "~a" " ")(format stream "~a" (quantity-name quantity))
  (cond ((not (null (quantity-in quantity)))
	 (format stream "~a" " in the ")
	 (format stream "~a"
		 (get-place-name-by-id (quantity-in quantity) places*))))
  (cond ((not (null (quantity-note quantity)))
	 (format stream "~%")(format stream "~a" "note")
	 (mapc #'(lambda (n) (format stream "~a" n)(format stream "~a" " "))
	       (quantity-note quantity)))))

(defun save-claims (stream)
  (mapc #'(lambda (c) (format stream "~%")(save-claim c stream))
	(reverse claims*)))

(defun save-claim (claim stream)
  (let* ((to nil)(to-name nil)(to-loc nil)
	 (from (get-quantity-by-id (claim-cause claim) quantities*))
	 (from-name (quantity-name from))
	 (from-loc (get-place-name-by-id (quantity-in from) places*)))
    (cond ((setq to (get-quantity-by-id (claim-effect claim) quantities*))
	  ...
	  (t (setq to (get-claim-by-id (claim-effect claim) claims*))
	     (setq to-name (claim-id to))))
    (format stream "~a" "claim ")(format stream "~a" to-name)
    (cond ((and (not (null to-loc))
		(not (eq to-loc from-loc)))
	   (format stream "~a" " in the ")(format stream "~a" to-loc)))
    (format stream "~a" " ")(format stream "~a" (claim-direction claim))
    (format stream "~a" " with ")(format stream "~a" from-name)
    (cond ((not (null from-loc))
	   (format stream "~a" " in the ")(format stream "~a" from-loc)))
    (cond ((not (null (claim-note claim)))
	   (format stream "~%")(format stream "~a" "note")
	   (mapc #'(lambda (n) (format stream "~a" " ")
		               (format stream "~a" n))
		 (claim-note claim))))))

(defun save-facts (stream)
  (mapc #'(lambda (f) (format stream "~%")(save-fact f stream))
	(reverse facts*)))

(defun save-fact (fact stream)
  (let* ((to (get-quantity-by-id (fact-to fact) quantities*))
	 (to-name (quantity-name to))
	 (to-loc (get-place-name-by-id (quantity-in to) places*))
	 (from (get-quantity-by-id (fact-from fact) quantities*))
	 (from-name (quantity-name from))
	 (from-loc (get-place-name-by-id (quantity-in from) places*)))
    (format stream "~a" "fact ")(format stream "~a" to-name)
    (cond ((not (eq to-loc from-loc))
	   (format stream "~a" " in the ")(format stream "~a" to-loc)))
    (format stream "~a" " ")(format stream "~a" (fact-direction fact))
    (format stream "~a" " with ")(format stream "~a" from-name)
    (cond ((not (null from-loc))
	   (format stream "~a" " in the ")(format stream "~a" from-loc)))
    (cond ((not (null (fact-note fact)))
	   (format stream "~%")(format stream "~a" "note")
	   (mapc #'(lambda (n) (format stream "~a" n)
		               (format stream "~a" " "))
		 (fact-note fact))))))
